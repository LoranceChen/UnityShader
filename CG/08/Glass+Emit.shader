Shader "Custom/unityCookie/08C.2-Glass+Emit" {
	Properties {
		...
		_EmitStrength("Emissive Strength",Range(0.0,2.0))=0
		}
	SubShader {
		Pass{
			...
			CGPROGRAM
				...
				float4 frag(vertexOutput i):COLOR
				{
					...
					float4 texE=tex2D(_EmitTex,i.tex.xy*_EmitTex_ST.xy+_EmitTex_ST.zw);
					//uncompress texN
...
//					normalDirection.z=localCoords.z*i.normalWorld;
					...
					float3 emitColor=texE.rgb*_EmitStrength;
					float3 colFinal=diffuseColor+specularColor * tex.a+rimColor+emitColor+UNITY_LIGHTMODEL_AMBIENT.rgb;
